...
CREATE TABLE IF NOT EXISTS sessions (id text PRIMARY KEY,machine_id uuid NOT NULL REFERENCES machines(id),project_id uuid NOT NULL REFERENCES projects(id),cwd text,worktree_name text,worktree_path text,is_worktree boolean NOT NULL DEFAULT false,source text NOT NULL,codex_version text,started_at timestamptz NOT NULL,ended_at timestamptz);
